...
export const defaultBloodPressure: BloodPressure = {
	systolic: 0,
	diastolic: 0,
};
...
export const normalizeBGRData = (
	bgrData: MeasurementMeanData
): MeasurementMeanData => {
	const { stressStatus, bloodPressureStatus } = bgrData;
	return {
		...bgrData,
		// @ts-ignore
		stressStatus:
			stressStatus === null || String(stressStatus) === NO_DATA
				? null
				: stressStatus,
		bloodPressureStatus:
			bloodPressureStatus === null || String(bloodPressureStatus) === NO_DATA
				? null
				: convertBPS(bloodPressureStatus),
	};
};

export const normalizeHRVData = (hrvMetrics: HrvMetrics): HrvMetrics => {
	const { sdnn } = hrvMetrics;
	return {
		...hrvMetrics,
...
export const checkIsAllDataCalculated = (rppgData: RPPGData) => {
	const { measurementData, bloodPressure } = rppgData;
	const { bpm, rr, stressStatus } = measurementData;
	const { systolic, diastolic } = bloodPressure;
	const isAllDataCalculated = Boolean(
		bpm && rr && stressStatus && systolic && diastolic
	);
	const threeValuesCalculated =
		[bpm, rr, stressStatus, systolic, diastolic].filter((item) => item)
			.length >= 3;
	return {
		isAllDataCalculated,
